...
public class KLongCubeMove : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        longNoteBeatLenght = Cubecreate.GetComponent<cubcreat>().KlongNoteBeatLenght;//方塊幾拍長
        ...
        StartPosition = Cubecreate.GetComponent<cubcreat>().KlongSpawn;//生成位置
        EndPosition = Cubecreate.GetComponent<cubcreat>().KlongRemove;//停止位置
        move = StartPosition;
    }
    bool firsthit = false;
    ...
    void Update()
    {
        if (gameObject.tag == "longnotes")
        {
            ...
            if (!firsthit)
            {
                if (Kpresstime < hittime + 0.125f && Kpresstime > hittime - 0.15f)
                {
